...
export function AnimatedAIInput() {
  ...
  const [workflowMode, setWorkflowMode] = useState<"ai" | "detailed">("ai");
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Navigate to project page with initial prompt and mode
      router.push(`/projects/${projectId}?prompt=${encodeURIComponent(value)}&mode=${workflowMode}`);
    } catch (error) {
  ...
  return (
    <div className="w-full max-w-[700px] lg:w-[700px] mx-auto py-6 px-4">
      {/* Mode Selector */}
      <div className="mb-4 flex items-center justify-center gap-2 p-1 bg-zinc-100 dark:bg-zinc-900 rounded-xl max-w-md mx-auto">
        <button
        ...
        <button
          onClick={() => setWorkflowMode("detailed")}
          className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg font-medium text-sm transition-all duration-200 ${
        ...
        <div className="bg-background/40 dark:bg-zinc-500/15 backdrop-blur-md rounded-[28px] border border-border/20 p-3 transition-all duration-300 ease-in-out">
          <div className="flex flex-col gap-2">
            ...
            >
              <div 
                className="relative"
                onMouseEnter={() => setIsModelSelectorHovered(true)}
                onMouseLeave={() => setIsModelSelectorHovered(false)}
              >
                <select
                  value={selectedModel}
                  onChange={(e) => setSelectedModel(e.target.value)}
                  className={`pl-3 pr-8 py-1.5 border rounded-xl text-xs font-medium cursor-pointer appearance-none transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-primary/50 ${
                    isModelSelectorHovered 
                      ? "bg-secondary/80 border-border text-foreground" 
                      : "bg-secondary/30 border-border/30 text-foreground/70"
                  }`}
                  style={{
                    backgroundImage: `url("data:image/svg+xml,%3Csvg width='12' height='12' viewBox='0 0 12 12' fill='${isModelSelectorHovered ? '%23666' : '%23999'}' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M3 4.5L6 7.5L9 4.5'/%3E%3C/svg%3E")`,
