...
export function TracesPanel({ isOpen, onClose }: TracesPanelProps) {
  ...
  useEffect(() => {
    if (!isOpen || !isLiveUpdating || !user?.uid) {
      console.log("TracesPanel - Not subscribing:", { isOpen, isLiveUpdating, hasUser: !!user?.uid });
      return;
    }

    setLoading(true);

    console.log("TracesPanel - Subscribing to agent traces for user:", user.uid);

    const unsubscribeTraces = subscribeToAgentTraces(
      user.uid,
      (newTraces) => {
        console.log("TracesPanel - Received traces:", newTraces.length);
        setTraces(newTraces);
        setLoading(false);
      },
    );
    ...
    const unsubscribeSpans = subscribeToAgentSpans(
      user.uid,
      (newSpans) => {
        console.log("TracesPanel - Received spans:", newSpans.length);
        setSpans(newSpans);
      },
    );
  ...
  }, [isOpen, isLiveUpdating, user?.uid, subscribeToAgentTraces, subscribeToAgentSpans]);
