...
function transformToFactoryConfig(workflowState: any): any {
  ...
  return {
    ...
    model_name: "gemini-2.5-flash",  // CORRECT model name from https://ai.google.dev/gemini-api/docs
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log("Export API - Verifying workflow with factory...");
    console.log("Factory URL:", FACTORY_URL);
    console.log("Built workflow:", JSON.stringify(builtWorkflow, null, 2));
    ...
    console.log("Export API - Factory response status:", res.status);
    
    const text = await res.text();
    console.log("Export API - Factory response text:", text);
    ...
    if (!res.ok) {
      console.error("Export API - Factory verification failed:", {
        status: res.status,
        data,
      });
      return new Response(
    ...
    console.log("Export API - Factory verification successful!");
