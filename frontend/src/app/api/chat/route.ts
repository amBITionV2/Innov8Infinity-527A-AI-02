...
export async function POST(req: NextRequest) {
  try {
    ...
    const { messages, projectId, userId, workflowMode = "ai" } = requestBody;
    ...
    const result = await streamText({
      model: google("gemini-2.5-flash"),  // CORRECT model from https://ai.google.dev/gemini-api/docs
      messages: modelMessages,
      system: getSystemPrompt(workflowMode),
      tools: {
