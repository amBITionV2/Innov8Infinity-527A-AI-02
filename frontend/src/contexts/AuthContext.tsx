  ...
  const getUserProjects = async (uid: string): Promise<Project[]> => {
    try {
      ...
      return snapshot.docs.map((doc) => {
        ...
        // Safely handle workflowState - it might be a string that needs parsing
        if (data.workflowState && typeof data.workflowState === 'string') {
          try {
            data.workflowState = JSON.parse(data.workflowState);
          } catch (e) {
            console.warn(`Failed to parse workflowState for project ${doc.id}:`, e);
      ...
      if (projectDoc.exists()) {
        ...
        if (data.workflowState && typeof data.workflowState === 'string') {
          try {
            ...
            console.warn(`Failed to parse workflowState for project ${projectDoc.id}:`, e);
            data.workflowState = null;
          }
        }
        ...
        if (docSnapshot.exists()) {
          ...
          if (data.workflowState && typeof data.workflowState === 'string') {
            try {
              ...
              console.warn(`Failed to parse workflowState for project ${docSnapshot.id}:`, e);
              data.workflowState = null;
            }
          }
